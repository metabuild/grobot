...
@TestFor(TargetNodeController)
@Mock(TargetNode)
class TargetNodeControllerTests {
    ...
    void testShow() {
        ...
        populateValidParams(params)
        def node = new TargetNode(params)
    ...
    void testDelete() {
        ...
        assert TargetNode.count() == 0
        assert TargetNode.get(node.id) == null
        assert response.redirectedUrl == '/node/list'
    }
}
